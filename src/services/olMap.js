            ...
            addGeometryToMap([
                {
                    ...
                    properties: { ...feature.properties, name: displayName },
                },
            ])
        }
        return feature
            ...
            addGeometryToMap([
                {
                    ...
                    properties: {
                        ...properties,
                        name: properties.display_name || properties.name || locationName,
                    },
                },
            ])
        }
...
const addGeometryToMap = (geometryData, clearPrevious = true) => {
    if (!_map) return
    ...
    // Normalizacja danych wejściowych do tablicy
    const items = Array.isArray(geometryData) ? geometryData : [geometryData]

    let allNewFeatures = []

    items.forEach((item) => {
        let geometry, locationName, properties

        // Obsługa różnych formatów danych wejściowych
        if (item.geometry) {
            // Format: { geometry, properties }
            geometry = item.geometry
            locationName = item.properties?.name || item.properties?.display_name || 'Obszar'
            properties = item.properties || {}
        } else {
            // Format: bezpośrednie przekazanie geometry (zachowanie wstecznej kompatybilności)
            geometry = item
            locationName = 'Obszar'
            properties = {}
        }
        ...
        allNewFeatures = allNewFeatures.concat(newFeatures)
    })

    _vectorLayer.getSource().addFeatures(allNewFeatures)
}
...
export const setStoryView = ({ features, feature, view = startView, sidebarWidthPx = 500 }) => {
    ...
    setTimeout(() => {
        ...
        if (features && Array.isArray(features)) {
            allFeatures.push(...features.filter((feat) => feat && feat.geometry))
        }

        // Wyświetlamy wszystkie features jednym wywołaniem
        if (allFeatures.length > 0) {
            addGeometryToMap(allFeatures, false)
        }
    }, 200)
}
