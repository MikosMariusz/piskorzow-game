    ...
    >
        ...
        >
            ...
            >
                ...
                >
                    <div
                        v-if="isGame && (!hasTask || isTaskCompleted)"
...
const goToPresentation = () => {
    ...
    localStorage.removeItem(`gameProgress_${props.storyId}_${context}`)
    router.push(isGame.value ? '/game' : '/presentation')
}

const saveGameProgress = () => {
    if (0 === props.activeIndex) return
    const context = isGame.value ? 'game' : 'presentation'
    const key = `gameProgress_${props.storyId}_${context}`
...
const beforeUnloadHandler = (e) => {
    if (props.activeIndex > 0 && !isTaskCompleted.value) {
        saveGameProgress()
        e.preventDefault()
        e.returnValue = ''
    }
